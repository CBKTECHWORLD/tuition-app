    ...
    <div className="auth-page">
      <div className="auth-left">
        <div className="auth-brand">
          <span className="brand-icon">◈</span> EduLearn
        </div>
        <h1 className="auth-headline">
          Learn from the<br /><em>best tutors</em><br />anywhere.
        </h1>
...
export function SignupPage() {
  ...
  const handleSignup = async (e) => {
    e.preventDefault();
    setError(''); setLoading(true);
    // Block anyone trying to manually set role to admin
    if (form.role === 'admin') {
      setError('Admin accounts cannot be created from signup. Contact the platform administrator.');
      setLoading(false);
      return;
    }
    try {
      ...
      await setDoc(doc(db, 'users', user.uid), {
        uid: user.uid, name: form.name,
        email: form.email, role: form.role,
        createdAt: serverTimestamp(),
        // Students are auto-approved, tutors need admin approval
    ...
    <div className="auth-page">
      <div className="auth-left">
        ...
        <div className="auth-roles">
          <div className="role-card"><span>🎓</span><strong>Student</strong><p>Join classes, watch videos, attend live sessions</p></div>
          <div className="role-card"><span>👨‍🏫</span><strong>Tutor</strong><p>Create classes, upload videos, host live sessions</p></div>
        </div>
      </div>
      <div className="auth-right">
        <div className="auth-card fade-in">
